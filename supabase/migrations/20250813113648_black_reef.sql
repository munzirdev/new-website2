...
-- إنشاء جدول file_attachments لحفظ الملفات كـ Base64 (تم إنشاؤه مسبقاً)
...
-- إنشاء فهارس لتحسين الأداء (تم إنشاؤها مسبقاً)
...
-- إنشاء RLS (Row Level Security) (تم تفعيله مسبقاً)
-- ALTER TABLE file_attachments ENABLE ROW LEVEL SECURITY;

-- سياسة الأمان: المستخدم يمكنه رؤية ملفاته فقط (تم إنشاؤها مسبقاً)
-- CREATE POLICY "Users can view their own file attachments" ON file_attachments
--     FOR SELECT USING (auth.uid() = user_id);

-- سياسة الأمان: المستخدم يمكنه إضافة ملفاته فقط (تم إنشاؤها مسبقاً)
-- CREATE POLICY "Users can insert their own file attachments" ON file_attachments
--     FOR INSERT WITH CHECK (auth.uid() = user_id);

-- سياسة الأمان: المستخدم يمكنه تحديث ملفاته فقط (تم إنشاؤها مسبقاً)
-- CREATE POLICY "Users can update their own file attachments" ON file_attachments
--     FOR UPDATE USING (auth.uid() = user_id);

-- سياسة الأمان: المستخدم يمكنه حذف ملفاته فقط (تم إنشاؤها مسبقاً)
-- CREATE POLICY "Users can delete their own file attachments" ON file_attachments
--     FOR DELETE USING (auth.uid() = user_id);

-- سياسة خاصة للإدارة: يمكنها رؤية جميع الملفات (تم إنشاؤها مسبقاً)
...
-- دالة لتحديث updated_at (تم إنشاؤها مسبقاً)
...
-- إنشاء trigger لتحديث updated_at (تم إنشاؤه مسبقاً)
